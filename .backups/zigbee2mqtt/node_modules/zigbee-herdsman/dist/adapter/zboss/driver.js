...
class ZBOSSDriver extends events_1.default {
    ...
    async execCommand(commandId, params = {}, timeout = 10000) {
        ...
        return this.queue.execute(async () => {
            ...
            frame.tsn = this.tsn;
            const waiter = this.waitFor(commandId, commandId == enums_1.CommandId.NCP_RESET ? null : this.tsn, timeout);
    ...
    async getCoordinatorVersion() {
        const ver = await this.execCommand(enums_1.CommandId.GET_MODULE_VERSION, {});
        const cver = await this.execCommand(enums_1.CommandId.GET_COORDINATOR_VERSION, {});
    ...
    async permitJoin(nwk, duration) {
        await this.execCommand(enums_1.CommandId.ZDO_PERMIT_JOINING_REQ, { nwk: nwk, duration: duration, tcSignificance: 1 });
    }
    async setTXPower(value) {
        await this.execCommand(enums_1.CommandId.SET_TX_POWER, { txPower: value });
    }
    async lqi(nwk, index) {
        return this.execCommand(enums_1.CommandId.ZDO_MGMT_LQI_REQ, { nwk: nwk, startIndex: index });
    }
    async neighbors(ieee) {
        return this.execCommand(enums_1.CommandId.NWK_GET_NEIGHBOR_BY_IEEE, { ieee: ieee });
    }
    async nodeDescriptor(nwk) {
        return this.execCommand(enums_1.CommandId.ZDO_NODE_DESC_REQ, { nwk: nwk });
    }
    async activeEndpoints(nwk) {
        return this.execCommand(enums_1.CommandId.ZDO_ACTIVE_EP_REQ, { nwk: nwk });
    }
    async simpleDescriptor(nwk, ep) {
        return this.execCommand(enums_1.CommandId.ZDO_SIMPLE_DESC_REQ, { nwk: nwk, endpoint: ep });
    }
    async removeDevice(nwk, ieee) {
        return this.execCommand(enums_1.CommandId.ZDO_MGMT_LEAVE_REQ, { nwk: nwk, ieee: ieee, flags: 0 });
    }
    async request(ieee, profileID, clusterID, dstEp, srcEp, data) {
        ...
        return this.execCommand(enums_1.CommandId.APSDE_DATA_REQ, payload);
    }
    async bind(destinationNetworkAddress, sourceIeeeAddress, sourceEndpoint, clusterID, destinationAddressOrGroup, type, destinationEndpoint) {
        return this.execCommand(enums_1.CommandId.ZDO_BIND_REQ, {
    ...
    async unbind(destinationNetworkAddress, sourceIeeeAddress, sourceEndpoint, clusterID, destinationAddressOrGroup, type, destinationEndpoint) {
        return this.execCommand(enums_1.CommandId.ZDO_UNBIND_REQ, {
            target: destinationNetworkAddress,
            srcIeee: sourceIeeeAddress,
            srcEP: sourceEndpoint,
            clusterID: clusterID,
